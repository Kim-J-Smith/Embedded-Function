...
- When an `ebd::fn_ref` is created by `ebd::make_fn` from a `std::cw<NTTP-Callable>` plus an object and the first parameter of `NTTP-Callable` is not a reference, the deduced signature is `const`-qualified, e.g. `ebd::make_fn<ebd::fn_ref>(std::cw<&free_func_add_ii>, a)` yields `ebd::fn_ref<int(int) const>`.

...
The deprecated `safe_fn` alias still works but has the following configuration: `IsThrowing=false, AssertObjectNoThrow=true`. If you migrate to `ebd::fn`, note that `AssertObjectNoThrow` changes to `false` — meaning `ebd::fn` no longer requires the callable object to be noexcept-constructible/destructible. If you need the `AssertObjectNoThrow=true` guarantee, use `ebd::basic_fn` directly:
